import streamlit as stimport librosa
import numpy as np
import pandas as pd
...
import os
import tempfile
import shutil# --- CONFIGURATION SYSTÈME ---
st.set_page_config(page_title="RCDJ228 MUSIC SNIPER", page_icon="", layout="wide")# Récupération des secrets
TELEGRAM_TOKEN = st.secrets.get("TELEGRAM_TOKEN")
CHAT_ID = st.secrets.get("CHAT_ID")# --- RÉFÉRENTIELS HARMONIQUES ---
NOTES_LIST = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
NOTES_ORDER = [f"{n} {m}" for n in NOTES_LIST for m in ['major', 'minor']]CAMELOT_MAP = {
...
st.markdown("""
    ...
    """, unsafe_allow_html=True)# --- MOTEURS DE CALCUL ---
...
def arbitrage_pivots_voisins(chroma_global, key_a, key_b, key_to_camelot_map):
    ...
    signatures = {
        ...
        '12A_vs_1A':  {'A#': '1A', 'A': '12A'},  # C#m vs G#m    # --- MAJEURS (B) ---
...
return Nonedef seconds_to_mmss(seconds):
    if seconds is None:
        return "??:??"
    mins = int(seconds // 60)
    secs = int(seconds % 60)
    return f"{mins:02d}:{secs:02d}"def apply_sniper_filters(y, sr):
    y_harm = librosa.effects.harmonic(y, margin=4.0)
    nyq = 0.5 * sr
    b, a = butter(4, [80/nyq, 5000/nyq], btype='band')
    return lfilter(b, a, y_harm)def get_bass_priority(y, sr):
    nyq = 0.5 * sr
    b, a = butter(2, 150/nyq, btype='low')
    y_bass = lfilter(b, a, y)
    chroma_bass = librosa.feature.chroma_cqt(y=y_bass, sr=sr, n_chroma=12)
    return np.mean(chroma_bass, axis=1)def detect_harmonic_sections(y, sr, duration, step=6, min_harm_duration=20, harm_threshold=0.3, perc_threshold=0.5):
    """
    ...
    flatness = librosa.feature.spectral_flatness(y=y)  # Haut pour percussion/voix parlée, bas pour harmonieharmonic_starts = []
...
return harm_start, harm_enddef detect_cadence_resolution(timeline, final_key):
    """
    ...
    subdom_idx = (root_idx + 5) % 12  # Sous-dominante (IV), optionnelresolution_count = 0
...
return cadence_scoredef solve_key_sniper(chroma_vector, bass_vector):
    best_overall_score = -1
    best_key = "Unknown"cv = (chroma_vector - chroma_vector.min()) / (chroma_vector.max() - chroma_vector.min() + 1e-6)
...
return {"key": best_key, "score": best_overall_score}def get_key_score(key, chroma_vector, bass_vector):
    note, mode = key.split()
    root_idx = NOTES_LIST.index(note)cv_norm = (chroma_vector - chroma_vector.min()) / (chroma_vector.max() - chroma_vector.min() + 1e-6)
...
return np.mean(profile_scores)def process_audio(audio_file, file_name, progress_placeholder):
    status_text = progress_placeholder.empty()
    progress_bar = progress_placeholder.progress(0)def update_prog(value, text):
...
return res_objdef get_chord_js(btn_id, key_str):
...
st.markdown("#### Système d'Analyse Harmonique 99% précis")# Ajout d'un placeholder pour le statut global en haut de la page
global_status = st.empty()uploaded_files = st.file_uploader(" Déposez vos fichiers (Audio)", type=['mp3','wav','flac','m4a'], accept_multiple_files=True, key="file_uploader")# Initialiser session_state pour stocker les analyses
if 'analyses' not in st.session_state:
    st.session_state.analyses = {}
if 'analyzing' not in st.session_state:
    st.session_state.analyzing = Falseif uploaded_files:
    global_status.info("Analyse des fichiers en cours...")
    progress_zone = st.container()# Boucle sur les fichiers en reversed pour cohérence
for i, f in enumerate(reversed(uploaded_files)):
    ...
    if file_name in st.session_state.analyses:
        ...
        with st.container():
            ...
            m2, m3 = st.columns(2)
            with m2: st.markdown(f"<div class='metric-box'><b>ACCORDAGE</b>
